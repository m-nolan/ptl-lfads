...
class DropChannels(object):
    ...
    def __call__(self,sample):
        _, n_ch = sample.shape
        n_ch_drop = torch.round(self.drop_ratio*n_ch)
...
class GooseWireless250(pl.LightningDataModule):
    ...
    def __init__(self, config, transforms=None):
        ...
        self.dims       = ( # use this to create model input sizes
            self.train_dims[0] + self.valid_dims[0] + self.test_dims[0],    # n_trial
            self.train_dims[1],                                             # n_sample
            self.train_dims[2]                                              # n_channel
        )
        self.transforms = None

    def prepare_data(self): # If I share this code, I'll want to expand this with a data download
        None
